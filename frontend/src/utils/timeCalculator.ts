...
export function liveWorkMin(
  ...
  if (input.isOt && !input.rawEnd) {
    ...
    const hasExtra = otStartDt != null && otEndDt.getTime() > otStartDt.getTime();
    const { extra1, extra2 } = hasExtra
      ? extraSplit(input.workDate, otStartDt, otEndDt)
      : { extra1: 0, extra2: 0 };
    ...
    return {
      ...
      otEnd: hasExtra ? formatDateTime(input.workDate, otEndDt) : null,
